defmodule Naive.Leader do
  ...
  def handle_call(
    ...
    case Enum.find_index(traders, &(&1.pid == trader_pid)) do
      nil ->
        Logger.warn("Tried to update the sate of trader that leader is not aware of")
  ...
  def handle_info(
    ...
    Logger.info("#{symbol} trader died - trying to restart")

    case Enum.find_index(traders, &(&1.pid == trader_pid)) do
      nil ->
        Logger.warn("Tried to restart finished #{symbol} but failed to find its cached state")
        {:noreply, state}
