defmodule BinanceMock do
  ...
  def order_limit_buy(symbol, qunatity, price, "GTC") do
  ...
  defp order_limit(symbol, quantity, price, side) do
    qunatity = Float.parse("#{quantity}") |> elem(0)
    price = Float.parse("#{price}") |> elem(0)

    %Binance.Order{} = fake_order = generate_fake_order(symbol, qunatity, price, side)

    GenServer.cast(__MODULE__, {:add_order, fake_order})

    {:ok, convert_order_to_otder_result(fake_order)}
  end
  ...
  defp subscribe_to_topic(symbol, subscriptions) do
    ...
    case Enum.member?(subscriptions, symbol) do
      false ->
        Logger.debug("BiannaceMock subscribing to #{stream_name}")
  ...
  defp convert_order_to_otder_result(%Binance.Order{} = order) do
    %{
      struct(
