defmodule Naive.Leader do
  ...
  # normal exit, start fresh trader
  def handle_info(
  ...
  # crash exit, start trader with cached state
  def handle_info(
  ...
  defp fresh_trader_state(settings) do
    %{struct(Trader.State, settings) | budget: Decimal.div(settings.budget, settings.chunks)}
  end
end
