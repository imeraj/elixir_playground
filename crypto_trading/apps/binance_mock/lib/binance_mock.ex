defmodule BinanceMock do
    ...
    order_book =
      if order.side == "SELL" do
        Map.replace!(
          order_book,
          :sell_side,
          [order | order_book.sell_side] |> Enum.sort(&Decimal.lt?(&1.price, &2.price))
        )
      else
        Map.replace!(
          order_book,
          :buy_side,
          [order | order_book.buy_side] |> Enum.sort(&Decimal.gt?(&1.price, &2.price))
        )
      end
  ...
  defp generate_fake_order(symbol, quantity, price, side) do
